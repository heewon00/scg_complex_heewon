  ...
  actions: {
    ...
    async getEmployees() {
      try {
        this.isLoading = true;
        const appname = 'query';
        this.employees = await fetchWrapper.get(baseUrl, appname);
    ...
    async addEmployee(employee) {
      try {
        ...
        const newEmployee = await fetchWrapper.post(baseUrl, appname, employee);
    ...
    async deleteEmployee(id) {
      try {
        this.isLoading = true;
        const appname = 'command';
        const deletedEmployee = await fetchWrapper.post(`/api/v1/employee/${id}`, appname);
    ...
    async editEmployee(id, employee) {
      ...
      try {
        this.isLoading = true;
        const appname = 'commands';
        let target = this.employees.find((t) => t.id === id);

        if (!target) return;

        const updatedEmployee = await fetchWrapper.post(`${baseUrl}/${id}`, appname, employee);
